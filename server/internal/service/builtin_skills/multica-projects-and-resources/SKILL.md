...
description: "Use when creating, inspecting, updating, or debugging Multica projects and project resources. Covers durable project context, git_repo and local_directory resources, how resources affect future agent task context, when to bind repos, and when not to mutate resources."
...
- `git_repo` — durable git repo context, with `resource_ref.url` and optional `default_branch_hint`;
...
multica project list --output json
multica project get <project-id> --output json
multica project create --title "<title>" --repo <repo-url> --output json
...
multica project resource update <project-id> <resource-id> --url <new-repo-url> --output json
...
Add/update a project resource when the user asks for durable project context: "把这个 git repo 绑到项目上", "以后都用这个 repo", "agent 总是拿不到这个项目的仓库", or "这个项目要在我的本地目录里跑".
...
3. Check `git_repo.resource_ref.url`, `default_branch_hint`, and `local_directory.resource_ref.daemon_id`.
