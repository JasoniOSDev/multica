...
describe("deriveGitLabSettings", () => {
  ...
  it("each sub-flag can be flipped independently when master is on", () => {
    ...
    ).toMatchObject({ enabled: true, mrSidebar: false, coAuthor: true, autoLinkMRs: true });

    expect(
      deriveGitLabSettings(ws({ co_authored_by_enabled: false })),
    ).toMatchObject({ enabled: true, mrSidebar: true, coAuthor: false, autoLinkMRs: true });

    expect(
      deriveGitLabSettings(ws({ gitlab_auto_link_mrs_enabled: false })),
    ).toMatchObject({ enabled: true, mrSidebar: true, coAuthor: true, autoLinkMRs: false });
  });
