...
export function CreateProjectModal({ onClose }: { onClose: () => void }) {
  ...
  // Repos selected to attach as git_repo resources after the project is
  ...
  // A project's source is binary: either a set of git repos OR a local
  ...
  const handleSubmit = async () => {
    ...
    let resources:
      | Array<{ resource_type: "git_repo" | "local_directory"; resource_ref: Record<string, unknown> }>
      | undefined;
    if (sourceMode === "repos" && selectedRepos.length > 0) {
      resources = selectedRepos.map((url) => ({
        resource_type: "git_repo" as const,
        resource_ref: { url },
      }));
    } else if (
      sourceMode === "local" &&
