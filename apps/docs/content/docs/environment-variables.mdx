...
The [GitLab MR ↔ issue integration](/gitlab-integration) is configured **per workspace** from **Settings → GitLab** — the base URL, webhook secret token, and an optional personal access token all live on the workspace connection, not in environment variables. Only one optional operator-level variable exists.

| Variable | Default | Description |
|---|---|---|
| `MULTICA_GITLAB_SECRET_KEY` | empty | Optional. Base64-encoded 32-byte key used to encrypt the per-workspace GitLab personal access token at rest. Generate with `openssl rand -base64 32`. Only needed if a workspace stores a PAT on its connection |

**Behavior when `MULTICA_GITLAB_SECRET_KEY` is unset:**

- The webhook flow (webhook URL + secret token + Merge request events) works unaffected — it does not use this key.
- Storing a personal access token on a GitLab connection requires this key. Without it, PAT storage is unavailable while the rest of the integration keeps working.
...
- [GitLab integration](/gitlab-integration) — how to connect a self-hosted GitLab instance and where the optional `MULTICA_GITLAB_SECRET_KEY` fits
