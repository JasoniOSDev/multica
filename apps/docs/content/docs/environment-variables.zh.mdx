...
[GitLab MR ↔ issue 集成](/gitlab-integration) 是**按 workspace** 在 **Settings → GitLab** 里配置的——base URL、webhook 密钥令牌、以及可选的 personal access token 都存在 workspace 连接上，而不是环境变量里。只有一个可选的运维层变量。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_GITLAB_SECRET_KEY` | 空 | 可选。base64 编码的 32 字节密钥，用于加密各 workspace GitLab personal access token 的静态存储。用 `openssl rand -base64 32` 生成。只有当某个 workspace 在连接上存了 PAT 时才需要 |

**`MULTICA_GITLAB_SECRET_KEY` 未设时：**

- webhook 流程（webhook URL + 密钥令牌 + Merge request events）不受影响——它不依赖这个 key。
- 在 GitLab 连接上存 personal access token 需要这个 key。没有它就无法存 PAT，集成其余部分照常工作。
...
- [GitLab 集成](/gitlab-integration) —— 怎么连自建 GitLab 实例，可选的 `MULTICA_GITLAB_SECRET_KEY` 用在哪
