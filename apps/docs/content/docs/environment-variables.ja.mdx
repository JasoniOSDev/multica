...
[GitLab MR ↔ イシュー連携](/gitlab-integration)は **workspace ごと**に **Settings → GitLab** から設定します — base URL、webhook シークレットトークン、任意の personal access token はすべて環境変数ではなく workspace の接続に保存されます。運用者レベルの任意変数は 1 つだけです。

| 変数 | デフォルト | 説明 |
|---|---|---|
| `MULTICA_GITLAB_SECRET_KEY` | 空 | 任意。workspace ごとの GitLab personal access token を保存時に暗号化するための、base64 エンコードされた 32 バイトのキー。`openssl rand -base64 32` で生成します。workspace が接続に PAT を保存する場合にのみ必要です |

**`MULTICA_GITLAB_SECRET_KEY` が設定されていない場合の動作:**

- webhook フロー（webhook URL + シークレットトークン + Merge request events）は影響を受けません — このキーを使いません。
- GitLab 接続に personal access token を保存するにはこのキーが必要です。なければ PAT の保存はできませんが、連携の残りの部分は通常どおり動作します。
...
- [GitLab 連携](/gitlab-integration) — セルフホストの GitLab インスタンスをどう接続するか、任意の `MULTICA_GITLAB_SECRET_KEY` はどこで使うか
