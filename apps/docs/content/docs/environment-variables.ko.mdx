...
[GitLab MR ↔ 이슈 연동](/gitlab-integration)은 **workspace 단위**로 **Settings → GitLab**에서 설정합니다 — base URL, webhook 시크릿 토큰, 선택적 personal access token은 모두 환경 변수가 아니라 workspace 연결에 저장됩니다. 운영자 수준 선택 변수는 단 하나뿐입니다.

| 변수 | 기본값 | 설명 |
|---|---|---|
| `MULTICA_GITLAB_SECRET_KEY` | 비어 있음 | 선택. workspace별 GitLab personal access token을 저장 시 암호화하는 데 쓰는 base64 인코딩된 32바이트 키. `openssl rand -base64 32`로 생성합니다. workspace가 연결에 PAT를 저장하는 경우에만 필요합니다 |

**`MULTICA_GITLAB_SECRET_KEY`가 설정되지 않았을 때의 동작:**

- webhook 흐름(webhook URL + 시크릿 토큰 + Merge request events)은 영향을 받지 않습니다 — 이 키를 사용하지 않습니다.
- GitLab 연결에 personal access token을 저장하려면 이 키가 필요합니다. 없으면 PAT 저장은 불가능하지만, 연동의 나머지는 정상적으로 동작합니다.
...
- [GitLab 연동](/gitlab-integration) — 자체 호스팅 GitLab 인스턴스를 어떻게 연결하는지, 선택적 `MULTICA_GITLAB_SECRET_KEY`는 어디에 쓰이는지
